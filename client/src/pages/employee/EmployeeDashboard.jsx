    ...
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="bg-white border-slate-200 shadow-sm">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
              ...
              <div className="text-3xl font-bold text-emerald-600">₹18,333.15</div>
        ...
        <Card className="md:col-span-2 bg-white border-slate-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Button variant="outline" size="sm" className="border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-50">View All</Button>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow className="border-slate-200">
                  ...
                  <TableHead className="text-slate-500 font-medium">Log In</TableHead>
                  <TableHead className="text-slate-500 font-medium">Log Out</TableHead>
              ...
              <TableBody>
                {attendanceLogs.map((log, i) => (
                  <TableRow key={i} className="border-slate-100 hover:bg-slate-50">
                    ...
                    <TableCell className="text-slate-700">{log.checkIn}</TableCell>
                    <TableCell className="text-slate-700">{log.checkOut}</TableCell>
                    <TableCell>
                      <span className={`text-sm font-medium ${log.status.includes('late') ? 'text-amber-600' : 'text-emerald-600'}`}>
                        {log.status}
                      </span>
                    </TableCell>
                  </TableRow>
      ...
      <Card className="bg-white border-slate-200 shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm">New Request</Button>
        </CardHeader>
